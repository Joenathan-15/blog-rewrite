...
@section('body')
    <form id="form_edit" class="bg-base-200 md:w-1/2 lg:w-2/5 p-5 mx-auto grid gap-5">
        <img id="image_preview"
            src="{{ $project->attachment_link ? $project->attachment_link : 'https://lh3.google.com/u/4/d/15Ha_JZRvEdj7rOEv1xXANj0zAQtsaR3z=w1920-h982-iv1' }}"
            class="h-64 w-full" alt="">
        <input id="attachment_link_txt" type="text" class="p-2 input input-bordered w-full rounded-xl"
            value="{{ $project->attachment_link }}" placeholder="Picture Link">
        <div class="grid">
        ...
        <input id="repository_txt" type="text" class="p-2 input input-bordered w-full rounded-xl"
            value="{{ $project->repository }}" placeholder="Repository">
        ...
        <select id="visibility_selelect" class="select select-bordered w-full">
            <option {{ $project->visibility ? '' : 'selected' }} value="0">Private</option>
            <option {{ $project->visibility ? 'selected' : '' }} value="1">Public</option>
        </select>
    ...
    <div class="bg-base-200 p-3 mt-5 rounded-xl">
        ...
        <div id="post_parent" class="grid gap-5 grid-cols-1 md:grid-cols-3 xl:grid-cols-4 mt-5">
            <a id="create_post" href="{{ route('admin.post.index', 'parent_uuid=' . $project->uuid) }}"
...
@push('scripts')
    <script>
        ...
        $('document').ready(function() {
            ...
            $.ajax({
                url: "{{ route('admin.post.find', $project->uuid) }}",
                method: "GET",
            ...
            $("#form_edit").on("submit", function(e) {
                ...
                var data = {
                    ...
                    "title": $('#title_txt').val(),
                    "repository": $('#repository_txt').val(),
                    "description": $("#description_txt").val(),
                    "visibility": $('#visibility_selelect').val(),
                    "_token": "{{ csrf_token() }}",
                }
                $.ajax({
                    url: "{{ route('admin.project.save', $project->uuid) }}",
                    method: "POST",
                    data: data,
                    success: function(response) {
                        edit_success_notification("Project")
                    },
                    error: function(error) {
            ...
            $('#delete_btn').on('click', function() {
                ...
                if (is_okay) {
                    $.ajax({
                        url: "{{ route('admin.project.delete', $project->uuid) }}",
                        method: "GET",
                        success: function(response) {
                            window.location.href = "{{ route('admin.dashboard') }}"
                            delete_success_notification("Project")
                        },
                        error: function(error) {
                            console.error(error);
                        }
